...
/// Public origin for canonicals / JSON-LD. Override with `SITE_URL` when the custom domain is live.
...
const TEXT: Landing = Landing {
    ...
    howto: [
        ...
        ("Copy or share", "Copy plain text or Markdown with timestamp links. The working URL is /?v={id}&mode=text (not indexed)."),
    ],
    ...
    why: [
        ...
        ("One result page", "After you paste, you stay on /?v={id}&mode=text. This landing is the guide; the result is the working document."),
...
const AUDIO: Landing = Landing {
    title: "YouTube to MP3 — Download Public Video Audio | YouTubeForge",
    description: "Download audio from a public YouTube video as MP3 (default), M4A, Opus, or WAV. Then open the transcript, SRT, or translation in the same app.",
    eyebrow: "YouTube → audio",
    h1: "Download audio from a public YouTube video",
    lead: "This page is for the soundtrack, not the transcript. We request an audio-only stream and let you save MP3 by default, or M4A, Opus, or WAV. Talks without captions can still have audio.",
...
const TRANSLATE: Landing = Landing {
    ...
    why: [
        ...
        ("Shareable language pair", "/?v={id}&mode=translate&lang=en&tlang=es keeps the same view. Result URLs stay noindex."),
